  ...
  <ccm-section>
    <div class="stack">
      <header class="view-header">
        <h1>Shortlist</h1>
        <p>Cards flagged for the upcoming newsletter. Confirm details or move them if priorities change.</p>
      </header>

      <div v-if="isLoadingCards && !shortlistCards.length" class="empty-state">
        <p>Loading shortlisted cards…</p>
      </div>

      <div v-else>
        <div v-if="!shortlistCards.length" class="empty-state">
          <p>No cards shortlisted yet.</p>
        </div>

        <div v-else class="cards">
          <ScrapeResultCard
            v-for="card in shortlistCards"
            :key="card.id"
