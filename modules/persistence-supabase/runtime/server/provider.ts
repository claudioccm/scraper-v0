...
import type { ScrapeResult } from '../../../scraper/runtime/types'
import type {
  ScraperPersistence,
  PersistenceMeta,
  PersistenceBlobs,
  PersistenceOptions,
  PersistenceResult,
  PersistenceStoredPaths
} from '../../../scraper/runtime/server/lib/persistence'
...
interface DbJoined extends DbRow {
  blobs: DbBlob[]
}
...
export function createSupabasePersistence(): ScraperPersistence {
  return {
    async upsertResult(meta: PersistenceMeta, blobs?: PersistenceBlobs, options?: PersistenceOptions): Promise<PersistenceResult> {
      ...
      const storedPaths: PersistenceStoredPaths = {}
    ...
    async findFreshByUrl(normalizedUrl: string, ttlMs: number): Promise<ScrapeResult | null> {
      ...
      return mapDbJoinedToScrapeResult(joined)
    }
  }
}
...
function mapDbJoinedToScrapeResult(row: DbJoined): ScrapeResult {
...
function resolveConfig(): StoreConfig | null {
  ...
  const supabase =
    cfg?.scraperPersistence?.supabase ||
    cfg?.scraper?.supabase ||
    cfg?.scraperSupabase
...
function warnMissingCredentials() {
  ...
  console.warn('[scraper-persistence] Supabase credentials missing; persistence disabled.')
}
